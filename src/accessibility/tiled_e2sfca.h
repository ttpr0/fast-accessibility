...
Vector<float> calcGRASP2SFCA(ITiledGraph* g, std::vector<Coord>& dem_points, std::vector<int>& dem_weights, std::vector<Coord>& sup_points, std::vector<int>& sup_weights,
                             int max_range)
{
    ...
    return calc2SFCA<GRASP>(solver, dem_n, dem_w, sup_n, sup_w, decay);
}
