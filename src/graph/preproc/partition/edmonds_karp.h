...
{
public:
    // has to be ptr to make class default initializable
    BaseGraphExplorer* expl;
    ...
    EdmondsKarp(BaseGraphExplorer* expl, std::span<int> sources, short source_tile, std::span<int> sinks, short sink_tile, std::span<int> base_nodes, short base_tile)
        : expl(expl), base_tile(base_tile), source_tile(source_tile), sink_tile(sink_tile), max_flow(0)
    {
        this->source_queue = std::queue<int>();
        this->node_tiles = std::vector<short>(expl->nodeCount(), 0);
        this->bfs_flags = std::vector<_Flag_EK>(expl->nodeCount(), {0});
        this->visited = std::vector<bool>(expl->nodeCount(), false);
        this->edge_flow = std::vector<std::array<char, 2>>(expl->edgeCount(), {0, 0});
    ...
    void ComputeMinCut()
    {
        ...
        std::vector<bool> visited(this->expl->nodeCount());

        // clear visited
        for (int i = 0; i < this->expl->nodeCount(); i++) {
            visited[i] = false;
        ...
        while (true) {
            ...
            this->expl->forAdjacentEdges(curr, Direction::FORWARD, Adjacency::ADJACENT_EDGES, [&visited, this, &queue](EdgeRef ref) {
            ...
            this->expl->forAdjacentEdges(curr, Direction::BACKWARD, Adjacency::ADJACENT_EDGES, [&visited, this, &queue](EdgeRef ref) {
                if (visited[ref.other_id] || this->node_tiles[ref.other_id] != this->base_tile) {
                    return;
                }
                auto flow = this->edge_flow[ref.edge_id];
    ...
    int BFS()
    {
        ...
        while (true) {
            ...
            this->expl->forAdjacentEdges(curr, Direction::FORWARD, Adjacency::ADJACENT_EDGES, [curr, &visited, this, &flags, &queue](EdgeRef ref) {
            ...
            this->expl->forAdjacentEdges(curr, Direction::BACKWARD, Adjacency::ADJACENT_EDGES, [curr, &visited, this, &flags, &queue](EdgeRef ref) {
                // check if edge should stil be traversed
                if (visited[ref.other_id]) {
                    return;
                }
