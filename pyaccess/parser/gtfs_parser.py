...
def read_trips(path: str, trips: dict[int, GTFSTrip], stop_locs: dict[int, Any]):
...
def read_trip_days(path: str, trips: dict[int, GTFSTrip]):
...
def read_stop_locations(path: str, filter: MultiPolygon):
...
def build_transit_graph(trips: dict[int, GTFSTrip], stop_locs: dict[int, StopLoc]) -> tuple[NodeVector, ConnectionVector, dict[str, list[list[tuple[int, int]]]]]:
    ...
    stop_vector = NodeVector()
    conn_vector = ConnectionVector()
    schedules_temp = []
    for s_id in store_order:
        stop = stops[s_id]
        stop_node = Node()
        stop_node.loc = Coord(stop.lon, stop.lat)
        stop_vector.append(stop_node)

        for neigh, conn_trips in stop.neighbours.items():
            n_id = id_mapping[neigh]
            conn = Connection(s_id, n_id, 0)
            conn_vector.append(conn)
            schedule = []
            for trip in conn_trips:
                schedule.append((trip[0], trip[1], trip[2]))
            schedules_temp.append(schedule)
    schedules: dict[str, list[list[tuple[int, int]]]] = {
        "monday": [[]]*len(conn_vector),
        "tuesday": [[]]*len(conn_vector),
        "wednesday": [[]]*len(conn_vector),
        "thursday": [[]]*len(conn_vector),
        "friday": [[]]*len(conn_vector),
        "saturday": [[]]*len(conn_vector),
        "sunday": [[]]*len(conn_vector),
    }
    for i, schedule in enumerate(schedules_temp):
        for day, dep, ar in schedule:
            ...
            if len(sc_e) == 0:
                ...
                sc_e.append((dep, ar))
    return stop_vector, conn_vector, schedules
...
def parse_gtfs(gtfs_path: str, filter_polygon: str) -> tuple[NodeVector, ConnectionVector, dict[str, list[list[tuple[int, int]]]]]:
    ...
    stop_locs = read_stop_locations(gtfs_path, filter)

    trips = {}
    read_trips(gtfs_path, trips, stop_locs)
    read_trip_days(gtfs_path, trips)

    return build_transit_graph(trips, stop_locs)
